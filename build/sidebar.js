...
var guildWarSubmenu = {
    ...
    "guild-war-dog-vh": makeGuildWarRaidBookmark(71864, 1, false),
    "guild-war-dog-ex": makeGuildWarRaidBookmark(71865, 1, false),
    "guild-war-dog-ex-plus": makeGuildWarRaidBookmark(71866, 1, false),
    "guild-war-hell-90": makeGuildWarRaidBookmark(71867, 1, true),
    "guild-war-hell-95": makeGuildWarRaidBookmark(71868, 1, true),
};
var menuItems = {
    "home": "/#mypage",
...
function isHorizontalLayout() {
    return isMobileSite() || currentSettings.horizontalBookmarks;
}
;
...
function _updateIconSize(menuIcon) {
    ...
    if (isHorizontalLayout())
        menuIconSize = iconRect.width * getEffectiveZoom(menuIcon);
    else
    ...
    */
    ...
    if (isHorizontalLayout()) {
        ...
        var trySpawn = function () {
            if (isShutdown)
                return true;
            var gameContainer = document.querySelector("#mobage-game-container");
            var wrapper = gameContainer.querySelector(".wrapper");
            if (!gameContainer || !wrapper)
                return false;
            var effectiveZoom = getEffectiveZoom(gameContainer) *
                getEffectiveZoom(document.documentElement);
            menuLeftEdge = wrapper.getBoundingClientRect().left * effectiveZoom;
            wrapper.style.marginTop = (48 / effectiveZoom).toFixed(0) + "px";
            return true;
        };
        if (!trySpawn())
...
function _doUpdateStatusPanel(panel, status) {
    ...
    _setWidth(panel.apMeter.fill, Math.floor(ap) / parseInt(status.max_ap));
    _setWidth(panel.apMeter.shadow, ap / parseInt(status.max_ap));
    panel.bpMeter.setAttribute("text", _roundDown(bp, 0));
    _setWidth(panel.bpMeter.fill, Math.floor(bp) / parseInt(status.max_bp));
    _setWidth(panel.bpMeter.shadow, bp / parseInt(status.max_bp));
...
function _updateItemsPanel(panel, uid) {
    ...
    chrome.runtime.sendMessage({ type: "getWatchedItems" }, function (watchedItems) {
        if (!currentSettings.itemsPanel)
            return;
        if (!watchedItems || !watchedItems.length)
            return;
...
function _doUpdateItemsPanel(_panel, uid, items) {
    ...
    chrome.runtime.sendMessage({ type: "getWatchedItems" }, function (watchedItems) {
        ...
        for (var i = 0, l = watchedItems.length; i < l; i++) {
            ...
            elt.style.left = x.toFixed(0) + "px";
            elt.style.top = y.toFixed(0) + "px";
            elt.title = itemInfo.name + " (right-click to hide)";
        ...
        if (isHorizontalLayout()) {
            panel.style.width = (Math.ceil(count / 2) * 24).toFixed(0) + "px";
        }
        else {
            panel.style.height = (Math.ceil(count / 2) * 32).toFixed(0) + "px";
        }
        _layoutPanels();
    });
}
