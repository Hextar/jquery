...
function refreshEnemyUI() {
    ...
    if (!enemyGauges)
        enemyGauges = document.querySelectorAll('div.btn-enemy-gauge:not(.prt-percent)');
    if (!enemyGauges)
        return;
        ...
        if (!container) {
            ...
            if (!paddingSource) {
                gauges[i].paddingSource = paddingSource =
                    document.querySelector("div.btn-enemy-gauge.prt-percent[target='" + gauge.targetIndex + "']");
            }
            if (paddingSource) {
