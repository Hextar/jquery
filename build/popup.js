...
function refreshStatus(status) {
    ...
    var apFill = Math.min((ap / parseInt(status.max_ap) * 100), 100).toFixed(1);
    var bpFill = Math.min(bp / status.max_bp * 100, 100).toFixed(1);
...
function useSoulBerry() {
    var useCount = Math.min(lastStatus.battle_point_limit, lastStatus.max_bp - lastStatus.bp);
    if (useCount < 1)
        useCount = 5;
